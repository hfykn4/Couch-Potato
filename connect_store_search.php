...
<style>
 body {
    ...
    th {
	background: linear-gradient(to bottom left, #ffcccc 15%, #666699 93%);
	color: #fff;
	font-size: 25px;
}
    th,
...
if (!$search1) {
	$sql = "SELECT * FROM store WHERE manager_staff_id = '$search2'";
} 
else if (!$search2) 
{
	$sql = "SELECT * FROM store WHERE store_id = '$search1'";
} else
{
	$sql = "SELECT * FROM store WHERE store_id = '$search1' AND manager_staff_id = '$search2'";
}
$result = $conn->query($sql);


if ($result->num_rows > 0) {
    echo "<table><tr><th>Store ID</th><th>Manager Staff ID </th>><th>Address ID </th><th>Last Update</th></tr>";
