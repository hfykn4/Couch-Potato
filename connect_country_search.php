...
<style>
 body {
    ...
    th {
	background: linear-gradient(to bottom left, #ff9999 -1%, #ffff99 100%);
	color: #fff;
	font-size: 25px;
}
    th,
...
if (!$search1) {
	$sql = "SELECT * FROM country WHERE country = '$search2'";
} 
else if (!$search2) 
{
	$sql = "SELECT * FROM country WHERE country_id = '$search1'";
} else
{
	$sql = "SELECT * FROM country WHERE country_id = '$search1' AND country = '$search2'";
}
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo "<table><tr><th>Country ID</th><th>Country</th><th>Last Update</th></tr>";
