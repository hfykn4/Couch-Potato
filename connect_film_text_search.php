...
<style>
 body {
    ...
    th {
	background: linear-gradient(to bottom left, #cc99ff -1%, #ff6666 100%);
	color: #fff;
	font-size: 25px;
}
    th,
...
if (!$search1) {
	$sql = "SELECT * FROM film_text WHERE title = '$search2'";
} 
else if (!$search2) 
{
	$sql = "SELECT * FROM film_text WHERE film_id = '$search1'";
} else
{
	$sql = "SELECT * FROM film_text WHERE film_id  = '$search1' AND title = '$search2'";
}
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo "<table><tr><th>Film ID</th><th>Title</th><th>Description</th></tr>";
