...
<style>
 body {
    ...
    th {
	background: linear-gradient(to top right, #009999 -1%, #ffffff 100%);
	color: #fff;
	font-size: 25px;
}
    th,
...
if (!$search1) {
	$sql = "SELECT * FROM film_category WHERE category_id = '$search2'";
} 
else if (!$search2) 
{
	$sql = "SELECT * FROM film_category WHERE film_id = '$search1'";
} else
{
	$sql = "SELECT * FROM film_category WHERE film_id = '$search1' AND category_id = '$search2'";
}
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo "<table><tr><th>Film ID</th><th>Category ID</th><th>Last Update</th></tr>";
